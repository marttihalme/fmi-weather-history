...
const YearCompare = {
  ...
  render() {
    ...
    if (winterData.length === 0 && anomalies.length === 0) {
      container.innerHTML = '<div style="padding: 2rem; color: #666;">No data available</div>';
      return;
    }
    ...
    years.forEach(year => {
      if (this.selectedZone === 'all') {
        ...
        const yearGroup = this.createYearGroupWithZones(year, winterData, anomalies);
        container.appendChild(yearGroup);
      } else {
        // Show single zone per row
        const row = this.createYearRow(year, this.selectedZone, winterData, anomalies);
  ...
  createYearGroupWithZones(year, winterData, anomalies) {
    ...
    zones.forEach((zone, zoneIndex) => {
      ...
      // Draw data after DOM render
      requestAnimationFrame(() => {
        this.drawYearData(svg, year, zone, winterData, anomalies, this.zoneRowHeight);
      });
  ...
  drawYearData(svg, year, zone, winterData, anomalies, height) {
      ...
      if (winter.cold_spells) {
        winter.cold_spells.forEach(spell => {
          ...
          if (x1 !== null && x2 !== null) {
            ...
            rect.setAttribute('width', Math.max(2, Math.min(width - x1, x2 - x1)));
    ...
    const relevantSlippery = slipperyData.filter(r => r.zone === zone && r.year === year);
      ...
      if (risk.slippery_periods) {
        risk.slippery_periods.forEach(period => {
          ...
          if (x1 !== null && x2 !== null) {
            ...
            rect.setAttribute('fill', isHighRisk ? 'rgba(255, 152, 0, 0.8)' : 'rgba(255, 193, 7, 0.6)');
  ...
  showSpellTooltip(event, spell, winter, type) {
    ...
    let html = '';
    const minTemp = spell.min_temp !== undefined && spell.min_temp !== null && !isNaN(spell.min_temp)
      ? spell.min_temp.toFixed(1) : '-';
    const maxTemp = spell.max_temp !== undefined && spell.max_temp !== null && !isNaN(spell.max_temp)
  ...
  showAnomalyTooltip(event, anomaly) {
    ...
    if (anomaly.min_temperature !== null && anomaly.min_temperature !== undefined && !isNaN(anomaly.min_temperature)) {
      html += `<br>Min: ${anomaly.min_temperature.toFixed(1)}°C`;
    }
    if (anomaly.max_temperature !== null && anomaly.max_temperature !== undefined && !isNaN(anomaly.max_temperature)) {
  ...
  showSlipperyTooltip(event, period, risk) {
    ...
    const isHighRisk = period.high_risk_days > 0;
    const avgMin = period.avg_min_temp !== null && period.avg_min_temp !== undefined
      ? period.avg_min_temp.toFixed(1) : '-';
    const avgMax = period.avg_max_temp !== null && period.avg_max_temp !== undefined
